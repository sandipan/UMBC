 ...
 * AvgLauncher
 *
 * Driver for Avg map/reduce
 *
 * @author(Sandipan)
 */
@SuppressWarnings("deprecation")
public class AvgLauncher extends Configured implements Tool 
{
   String inFile;
   String outDir;
   int numMaps;
   ...
   public AvgLauncher(String inFile, String outDir, int numMaps, int numReduces, boolean split)
   {
	  this.inFile = inFile;
	  this.outDir = outDir;
  ...
  {
    JobConf conf = new JobConf(getConf(), AvgLauncher.class);
    ...
    conf.setMapperClass(split ? AvgSCalculatorMapper.class : AvgCalculatorMapper.class);
    //conf.setCombinerClass(AvgCalculatorReducer.class); 
    conf.setReducerClass(AvgCalculatorReducer.class);
    ...
    if (split)
    {
	    ...
	    try 
	    {
	      ...
	      {
	    	  Util.println(key.toString() + ": " + value.toString());
	      }
	    } 
	    finally 
	    {
